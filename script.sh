...
if [[ `echo "$DISKS"| wc -l` > 1 ]]; then
  mailx -s "Disk threshold - Above 90%" hgunti7@gmail.com test <<EOF
...
if [[ `echo "$CPU"| wc -l` > 3 ]]; then
  mailx -s "CPU threshold - Above 95%" bk71979@gmail.com test <<EOF
...
if [[ $MEM > 95 ]]; then
  mailx -s "Memory threshold - Total available less than 5%" bk71979@gmail.com test <<EOF
